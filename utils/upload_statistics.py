...
class UploadStatistics:
    ...
    def save_upload_record(self, task):
        """保存上传记录
        ...
        try:
            with self.lock:
                # 获取当前日期键
                day_key = task.create_time.strftime('%Y-%m-%d')
                ...
                # 将datetime类型转换为时间戳
                create_time = time.mktime(task.create_time.timetuple())
                update_time = time.time()
                
                # 获取各阶段时间，并转换为时间戳
                upload_start_time = time.mktime(task.upload_start_time.timetuple()) if task.upload_start_time else None
                upload_end_time = time.mktime(task.upload_end_time.timetuple()) if task.upload_end_time else None
                process_start_time = time.mktime(task.process_start_time.timetuple()) if task.process_start_time else None
                process_end_time = time.mktime(task.process_end_time.timetuple()) if task.process_end_time else None
                publish_start_time = time.mktime(task.publish_start_time.timetuple()) if task.publish_start_time else None
                publish_end_time = time.mktime(task.publish_end_time.timetuple()) if task.publish_end_time else None
                
                video_url = task.video_url
                publish_id = getattr(task, 'publish_id', None)
                extra_data = json.dumps({
                    'progress': task.progress,
                    'retry_count': task.retry_count,
                    'status_history': [
                        {
                            'time': h['time'].strftime('%Y-%m-%d %H:%M:%S') if isinstance(h['time'], datetime) else h['time'],
                            'status': h['status'],
                ...
                # 检查是否存在记录
                cursor = self.conn.cursor()
                cursor.execute('SELECT id FROM upload_records WHERE trace_id = ?', (trace_id,))
                ...
                if record:
                    ...
                    WHERE trace_id = ?
                    ''', (
                        status, error, error_detail, error_code, update_time,
                        upload_start_time, upload_end_time, process_start_time, process_end_time,
                        publish_start_time, publish_end_time, video_url, publish_id, extra_data,
                        trace_id
                    ))
                else:
                    # 插入记录
                    ...
                    INSERT INTO upload_records (
                        trace_id, account_id, file_path, file_name, file_size,
                        status, error, error_detail, error_code,
                        create_time, update_time,
                        upload_start_time, upload_end_time,
                        process_start_time, process_end_time,
                        publish_start_time, publish_end_time,
                        video_url, publish_id, day_key, extra_data
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        trace_id, account_id, file_path, file_name, file_size,
                        status, error, error_detail, error_code,
