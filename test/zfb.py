        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for i in range(num_parts):
                ...
                futures.append(executor.submit(
                    upload_part,
                    mt, file_id, file_path, start_pos, max_size, i + 1
                ))
...
def upload_part(mt, file_id, file_path, start_pos, chunk_size, part_num):
    """单个分块上传的实现"""
...
def get_app_id(cookies_str):
    """
    添加缓存以避免重复请求
    注意：需要将cookies转换为字符串作为缓存key
    """
...
def process_single_video(args):
    ...
    # 获取对应的图片路径
    pic_path = os.path.splitext(file_path)[0] + '.jpg'
    ...
    for attempt in range(retries):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                # 同时提交视频和封面上传任务
                video_future = executor.submit(upload_4m_video, mt, file_path)
                cover_future = executor.submit(upload_pic, cookies, file_path)
                
                # 获取结果
                file_id, videoFileName = video_future.result()
                extProperty = cover_future.result()

            # 并行获取appid和视频URL
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                appid_future = executor.submit(get_app_id, cookies)
                video_url_future = executor.submit(get_video_url, file_id, mt)
                
                appid = appid_future.result()
                videoFile = video_url_future.result()
            ...
            if signal is not None and index is not None:
                signal.emit(index)
            print(f"Successfully processed and deleted: {file_path} and {pic_path}")
            return True
